...
- 🎯 Entry points to execute your program using `python -m <wubby_python_api>` or `$ wubby_python_api` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/choke-dev/wubby-python-api/branch/main/graph/badge.svg?token=wubby-python-api_token_here)](https://codecov.io/gh/choke-dev/wubby-python-api)
[![CI](https://github.com/choke-dev/wubby-python-api/actions/workflows/main.yml/badge.svg)](https://github.com/choke-dev/wubby-python-api/actions/workflows/main.yml)

Awesome wubby_python_api created by choke-dev

## Install

from source
```bash
git clone https://github.com/choke-dev/wubby-python-api wubby_python_api
cd wubby_python_api
make install
```
...
```bash
pip install wubby_python_api
```

## Executing

```bash
$ wubby_python_api run --port 8080
```

or

```bash
python -m wubby_python_api run --port 8080
```

or

```bash
$ uvicorn wubby_python_api:app
```

## CLI

```bash
❯ wubby_python_api --help
Usage: wubby_python_api [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ wubby_python_api create-user --help
Usage: wubby_python_api create-user [OPTIONS] USERNAME PASSWORD
...
```bash
wubby_python_api create-user admin admin --superuser
```
...
Run with `wubby_python_api run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `wubby_python_api create-user`.
...
-----------------------------------------------------
wubby_python_api/__init__.py              4      0   100%
wubby_python_api/app.py                  16      1    94%
wubby_python_api/cli.py                  21      0   100%
wubby_python_api/config.py                5      0   100%
wubby_python_api/db.py                   10      0   100%
wubby_python_api/models/__init__.py       0      0   100%
wubby_python_api/models/content.py       47      1    98%
wubby_python_api/routes/__init__.py      11      0   100%
wubby_python_api/routes/content.py       52     25    52%
wubby_python_api/routes/security.py      15      1    93%
wubby_python_api/routes/user.py          52     26    50%
wubby_python_api/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in wubby_python_api/default.toml.
